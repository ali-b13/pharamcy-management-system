...
const WarningList = () => {
    ...
    const fetchMedicines = useCallback(async (page: number) => {
        ...
        setError(null);
        try {
            const data = await getMedicineWarnings(page, 4);
            setMedicines(data.warningMedicines);
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen md:max-w-3xl md:mx-auto md:my-8 rounded-2xl shadow-lg">
            <Header title="الأدوية المنتهية قريبًا" />
                ...
                {loading ? (
                    ...
                    <>
                        <ul className="space-y-4">
                            {warningMedicines?.length ? (
                                warningMedicines.map((batch: BatchWithMedicineProps) => (
                                    <li key={batch.batchNumber} className="p-5 bg-yellow-50 rounded-lg shadow-sm border border-yellow-200 hover:shadow-md transition-shadow duration-200">
                                        <div className="flex items-center justify-start gap-3 mb-6">
                                            <div className="text-lg font-bold text-yellow-700">الدواء: {batch.medicine.name}</div>
                                            <div className="text-sm font-medium text-yellow-600">الشركة: {batch.medicine.brand}</div>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-700">
                                            ...
                                            <div className="flex items-center col-span-2">
                                                ...
                                                <span className="w-2/3 text-yellow-700 font-semibold">
                                ...
                                <div className="text-center text-gray-600 py-6">لا توجد أدوية تحذيرية</div>
                            )}
                        </ul>
                        <div className="mt-6 flex justify-center">
                            {warningMedicines && warningMedicines.length > 0 && (
                                <Pagination
