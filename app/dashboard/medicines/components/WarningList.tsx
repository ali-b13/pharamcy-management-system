...
import { WarningProps } from './WarningOverview';
...
const WarningList = ({ medicines, totalPages }: WarningListProps) => {
    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen md:max-w-3xl  md:my-8 rounded-2xl shadow-lg">
            <Header title='الأدوية المنتهية قريباّ' />
            <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                <ul className="space-y-4">
                    {warningMedicines?.length ? warningMedicines.map((batch: BatchWithMedicineProps) => (
                        <li key={batch.batchNumber} className="p-5 bg-yellow-50 rounded-lg shadow-sm border border-yellow-300">
                            <div className="flex items-center justify-start gap-3  mb-6">
                            <div className="text-lg font-bold text-yellow-600 "> الدواء : {batch.medicine.name}</div>
                            <div className="text-sm font-medium text-yellow-500"> الشركه : {batch.medicine.brand}</div>
                            </div>
                    ...
                    )) : <div className="text-center text-gray-600">لا توجد أدوية تحذيرية</div>}
                </ul>
